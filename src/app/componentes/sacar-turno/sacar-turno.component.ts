  ...
  sacarTurno() {
    ...
    this.regService.obtenerXTodos("usuarios/" + this.turno.especialista + "/turnos").subscribe((auxDatos) => {
      auxDatos.forEach((response: any) => {
        ...
        if (fechaInfo.fecha == this.turno.fecha && fechaInfo.hora == this.turno.hora ) {
    ...
    this.turnoCompleto = { cliente: this.mail, especialista: this.turno.especialista, fecha: this.turno.fecha, hora: this.turno.hora ,sala:  saloncito}
